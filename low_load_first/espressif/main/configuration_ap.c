...
void start_access_point()
{
  ...
  snprintf(ap.ssid, 32, "%s-%02X%02X%02X", "VolcConvAI", mac[3], mac[4], mac[5]);
  ...
  strncpy((char *)wifi_config.ap.ssid, ap.ssid, sizeof(wifi_config.ap.ssid));
  wifi_config.ap.ssid_len = strlen(ap.ssid);
  wifi_config.ap.max_connection = 4;
  wifi_config.ap.authmode = WIFI_AUTH_WPA2_PSK;
  strncpy((char *)wifi_config.ap.password, ap.ssid, sizeof(wifi_config.ap.password) - 1);
...
void url_decode(char *url, int url_len, char *decoded, int decoded_len)
{
  if (url == NULL || decoded == NULL || url_len == 0 || decoded_len == 0 || url_len > decoded_len) {
    return;
  }
  int len = strlen(url);
...
esp_err_t handler_form_submission(httpd_req_t *r)
{
  ...
  char decoded[128];
  url_decode(buf, sizeof(buf), decoded, sizeof(decoded));
  ...
  ssid[32] = '\0';
  password[64] = '\0';
  if (strlen(ssid) == 0 || strlen(password) == 0 || strlen(password) > 64 || strlen(ssid) > 32) {
    httpd_resp_send_err(r, HTTPD_400_BAD_REQUEST, "Invalid form data");
    return ESP_FAIL;
  }
