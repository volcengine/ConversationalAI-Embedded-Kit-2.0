...
2. 修复乐鑫按键问题
   ...
   ```bash
   git apply $ADF_PATH/examples/ConversationalAI-Embedded-Kit-2.0/examples/low_load_solution/espressif/0001-esp_peripherals-Support-ADC-API-of-IDF-5.x-in-adc_bu.patch
   ```

### 编译固件

进入 `esp-adf/examples/ConversationalAI-Embedded-Kit-2.0/examples/low_load_solution/espressif 目录下编译固件。

1. 进入 espressif 目录。
   
   ```bash
   cd $ADF_PATH/examples/ConversationalAI-Embedded-Kit-2.0/examples/low_load_solution/espressif
   ```
2. 设置编译目标平台。
